<!doctype html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <title>后台系统管理</title>
    <meta name="renderer" content="webkit|ie-comp|ie-stand">
    <meta http-equiv="X-UA-Compatible" content="IE=edge,chrome=1">
    <meta name="viewport" content="width=device-width,user-scalable=yes, minimum-scale=0.4, initial-scale=0.8"/>
    <meta http-equiv="Cache-Control" content="no-siteapp"/>
    <link rel="stylesheet" href="//cdn.junn.top/layui/2.4.3/css/layui.css">
    <link rel="stylesheet" href="./css/xadmin.css">
    <script type="text/javascript" src="//cdn.junn.top/jquery/3.3.1/jquery.min.js"></script>
    <script src="//cdn.junn.top/layui/2.4.3/layui.js" charset="utf-8"></script>
    <script type="text/javascript" src="./js/xadmin.js"></script>

</head>

<body>
<!-- 顶部开始 -->
<div class="container">
    <div class="logo"><a href="./index.html">后台管理系统</a></div>
    <div class="left_open">
        <i title="展开左侧栏" class="layui-icon layui-icon-shrink-right"></i>
    </div>
    <ul class="layui-nav left fast-add" lay-filter="">
        <li class="layui-nav-item">
            <a href="javascript:;">+新增</a>
            <dl class="layui-nav-child">
                <!-- 二级菜单 -->
                <dd><a onclick="x_admin_show('资讯','http://www.baidu.com')"><i class="layui-icon">&#xe6a2;</i>资讯</a></dd>
                <dd><a onclick="x_admin_show('图片','http://www.baidu.com')"><i class="layui-icon">&#xe6a8;</i>图片</a></dd>
                <dd><a onclick="x_admin_show('用户','http://www.baidu.com')"><i class="layui-icon">&#xe6b8;</i>用户</a></dd>
            </dl>
        </li>
    </ul>
    <ul class="layui-nav right" lay-filter="">
        <li class="layui-nav-item">
            <a href="javascript:;">${user.username}</a>
            <dl class="layui-nav-child">
                <!-- 二级菜单 -->
                <dd><a onclick="x_admin_show('个人信息','http://www.baidu.com')">个人信息</a></dd>
                <dd><a onclick="x_admin_show('切换帐号','http://www.baidu.com')">切换帐号</a></dd>
                <dd><a href="/logout">退出</a></dd>
            </dl>
        </li>
        <li class="layui-nav-item to-index"><a href="/">前台首页</a></li>
    </ul>

</div>
<!-- 顶部结束 -->
<!-- 中部开始 -->
<!-- 左侧菜单开始 -->
<div class="left-nav">
    <div id="side-nav">
        <ul id="nav">
            <li>
                <a href="javascript:;">
                    <i class="layui-icon layui-icon-user
"></i>
                    <cite>会员管理</cite>
                    <i class="layui-icon layui-icon-left nav_right"></i>
                </a>
                <ul class="sub-menu">
                    <li>
                        <a _href="member-list.html">
                            <i class="layui-icon layui-icon-right"></i>
                            <cite>会员列表</cite>
                        </a>
                    </li>
                    <li>
                        <a _href="member-del.html">
                            <i class="layui-icon layui-icon-right"></i>
                            <cite>会员删除</cite>
                        </a>
                    </li>
                <#--<li>
                    <a href="javascript:;">
                        <i class="layui-icon">&#xe70b;</i>
                        <cite>会员管理</cite>
                        <i class="layui-icon nav_right layui-icon-left"></i>
                    </a>
                    <ul class="sub-menu">
                        <li>
                            <a _href="xxx.html">
                                <i class="layui-icon layui-icon-right"></i>
                                <cite>会员列表</cite>

                            </a>
                        </li>
                        <li>
                            <a _href="xx.html">
                                <i class="layui-icon layui-icon-right"></i>
                                <cite>会员删除</cite>

                            </a>
                        </li>
                        <li>
                            <a _href="xx.html">
                                <i class="layui-icon layui-icon-right"></i>
                                <cite>等级管理</cite>

                            </a>
                        </li>
                    </ul>
                </li>-->
                </ul>
            </li>
            <li>
                <a href="javascript:;">
                    <i class="layui-icon layui-icon-cart-simple"></i>
                    <cite>订单管理</cite>
                    <i class="layui-icon nav_right layui-icon-left"></i>
                </a>
                <ul class="sub-menu">
                    <li>
                        <a _href="order-list.html">
                            <i class="layui-icon layui-icon-right"></i>
                            <cite>订单列表</cite>
                        </a>
                    </li>
                </ul>
            </li>
            <li>
                <a href="javascript:;">
                    <i class="layui-icon layui-icon-username"></i>
                    <cite>管理员管理</cite>
                    <i class="layui-icon nav_right layui-icon-left"></i>
                </a>
                <ul class="sub-menu">
                    <li>
                        <a _href="admin-list.html">
                            <i class="layui-icon layui-icon-right"></i>
                            <cite>管理员列表</cite>
                        </a>
                    </li>
                    <li>
                        <a _href="admin-role.html">
                            <i class="layui-icon layui-icon-right"></i>
                            <cite>角色管理</cite>
                        </a>
                    </li>
                    <li>
                        <a _href="admin-cate.html">
                            <i class="layui-icon layui-icon-right"></i>
                            <cite>权限分类</cite>
                        </a>
                    </li>
                    <li>
                        <a _href="admin-rule.html">
                            <i class="layui-icon layui-icon-right"></i>
                            <cite>权限管理</cite>
                        </a>
                    </li>
                </ul>
            </li>
            <li>
                <a href="javascript:;">
                    <i class="layui-icon layui-icon-chart-screen"></i>
                    <cite>系统统计</cite>
                    <i class="layui-icon nav_right layui-icon-left"></i>
                </a>
                <ul class="sub-menu">
                    <li>
                        <a _href="echarts1.html">
                            <i class="layui-icon layui-icon-right"></i>
                            <cite>拆线图</cite>
                        </a>
                    </li>
                    <li>
                        <a _href="echarts2.html">
                            <i class="layui-icon layui-icon-right"></i>
                            <cite>柱状图</cite>
                        </a>
                    </li>
                    <li>
                        <a _href="echarts3.html">
                            <i class="layui-icon layui-icon-right"></i>
                            <cite>地图</cite>
                        </a>
                    </li>
                    <li>
                        <a _href="echarts4.html">
                            <i class="layui-icon layui-icon-right"></i>
                            <cite>饼图</cite>
                        </a>
                    </li>
                    <li>
                        <a _href="echarts5.html">
                            <i class="layui-icon layui-icon-right"></i>
                            <cite>雷达图</cite>
                        </a>
                    </li>
                    <li>
                        <a _href="echarts6.html">
                            <i class="layui-icon layui-icon-right"></i>
                            <cite>k线图</cite>
                        </a>
                    </li>
                    <li>
                        <a _href="echarts7.html">
                            <i class="layui-icon layui-icon-right"></i>
                            <cite>热力图</cite>
                        </a>
                    </li>
                    <li>
                        <a _href="echarts8.html">
                            <i class="layui-icon layui-icon-right"></i>
                            <cite>仪表图</cite>
                        </a>
                    </li>
                </ul>
            </li>
        </ul>
    </div>
</div>
<!-- <div class="x-slide_left"></div> -->
<!-- 左侧菜单结束 -->
<!-- 右侧主体开始 -->
<div class="page-content">
    <div class="layui-tab tab" lay-filter="xbs_tab" lay-allowclose="false">
        <ul class="layui-tab-title">
            <li>我的桌面</li>

        </ul>
        <div class="layui-tab-content">
            <div class="layui-tab-item layui-show">
                <iframe src='./welcome.html' frameborder="0" scrolling="yes" class="x-iframe"></iframe>
            </div>
        </div>
    </div>
</div>
<div class="page-content-bg"></div>
<!-- 右侧主体结束 -->
<!-- 中部结束 -->
<!-- 底部开始 -->
<div class="footer">
    <div class="copyright">Copyright ©2017 All Rights Reserved</div>
</div>
<!-- 底部结束 -->

</body>

</html>